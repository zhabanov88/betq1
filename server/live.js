'use strict';
/**
 * BetQuant Pro — Live Monitor API  /api/live/*
 *
 * GET  /api/live/matches           — активные + запланированные матчи
 * GET  /api/live/match/:id         — детали: события, статистика, сигнал
 * GET  /api/live/odds/:id          — история коэффициентов (полная)
 * POST /api/live/refresh           — сброс кэша
 *
 * Источники:
 *  • Если задан API_FOOTBALL_KEY  → api-football.com (100 req/day free)
 *  • Иначе                        → встроенные demo-данные (всё работает)
 */
  ...
  return [
    {
      ...
      events: [
        { minute: 12, type: 'goal',   team: 'home', player: 'Saka',    assist: 'Ødegaard', score: '1:0' },
        { minute: 38, type: 'yellow', team: 'away', player: 'E. Fernández' },
        { minute: 51, type: 'goal',   team: 'away', player: 'Palmer',  assist: 'Jackson',  score: '1:1' },
        { minute: 63, type: 'goal',   team: 'home', player: 'Havertz', assist: 'Saka',     score: '2:1' },
      ],
    },
    {
      id: 'lv2', sport: 'football', status: 'live', minute: 34,
      league: 'La Liga', country: 'Spain',
      home: 'Real Madrid', homeScore: 0, homeLogo: '⚪',
      away: 'Barcelona',   awayScore: 0, awayLogo: '🔵',
      startTime: ago(34),
      odds: { home: 2.40, draw: 3.10, away: 2.95, over25: 1.75, under25: 2.05 },
      openOdds: { home: 2.10, draw: 3.40, away: 3.30 },
      stats: {
        home_shots: 7,  away_shots: 11,
        home_sot: 2,    away_sot: 5,
        home_poss: 44,  away_poss: 56,
        home_corners: 2, away_corners: 6,
        home_xg: 0.61,  away_xg: 1.42,
        home_da: 19,    away_da: 31,
      },
      events: [
        { minute: 18, type: 'yellow', team: 'home', player: 'Tchouaméni' },
        { minute: 29, type: 'yellow', team: 'away', player: 'Koundé' },
      ],
    },
    {
      id: 'lv3', sport: 'hockey', status: 'live', minute: 38,
      league: 'NHL', country: 'USA',
      home: 'Rangers', homeScore: 3, homeLogo: '🗽',
      away: 'Bruins',  awayScore: 2, awayLogo: '🐻',
      period: '2nd', startTime: ago(38),
      odds: { home: 1.55, away: 2.40, over55: 1.85, under55: 1.95 },
      openOdds: { home: 1.80, away: 2.05 },
      stats: {
        home_shots: 21, away_shots: 18,
        home_sot: 21,   away_sot: 18,
        home_poss: 52,  away_poss: 48,
        home_xg: 3.10,  away_xg: 2.50,
        home_da: 0,     away_da: 0,
        home_corners: 0, away_corners: 0,
      },
      events: [
        { minute: 8,  type: 'goal', team: 'home', player: 'Panarin',    score: '1:0' },
        { minute: 19, type: 'goal', team: 'away', player: 'Pastrnak',   score: '1:1' },
        { minute: 22, type: 'goal', team: 'home', player: 'Zibanejad',  score: '2:1' },
        { minute: 31, type: 'goal', team: 'away', player: 'Marchand',   score: '2:2' },
        { minute: 36, type: 'goal', team: 'home', player: 'Kreider',    score: '3:2' },
      ],
    },
    {
      id: 'sc1', sport: 'football', status: 'scheduled', minute: 0,
      league: 'Bundesliga', country: 'Germany',
      home: 'Bayern',   homeScore: null, homeLogo: '🔴',
    ...
    {
      id: 'sc2', sport: 'basketball', status: 'scheduled', minute: 0,
      league: 'NBA', country: 'USA',
      home: 'Lakers',   homeScore: null, homeLogo: '💜',
      away: 'Warriors', awayScore: null, awayLogo: '💛',
      startTime: fwd(4),
      odds: { home: 1.85, away: 1.95, over_total: 1.90, under_total: 1.90 },
      openOdds: { home: 1.85, away: 1.95 },
      stats: {}, events: [],
    },
  ];
}

// ─── Odds history (pre-match + live) ─────────────────────────────────────
function buildOddsHistory(m) {
  const pts = [];
  const base = m.openOdds;
  const start = new Date(m.startTime).getTime();

  // Pre-match: 24h → kick-off (10 точек)
  for (let h = 24; h >= 0; h -= 3) {
    const n = () => (Math.random() - .5) * .08;
    pts.push({
      t: new Date(start - h * 3600000).toISOString(),
      phase: 'pre',
      home: +(base.home  + n()).toFixed(2),
      draw: base.draw ? +(base.draw + n()).toFixed(2) : null,
      away: +(base.away  + n()).toFixed(2),
    });
  }

  if (m.status !== 'live') return pts;

  // Live: каждые 5 минут до текущей
  for (let min = 0; min <= m.minute; min += 5) {
    ...
    pts.push({
      t: new Date(start + min * 60000).toISOString(),
      phase: 'live', minute: min,
...
function inPlaySignal(m) {
  ...
  if (diff < 0 && pH > pA * 1.4 && rem > 20)
    signals.push({
      ...
      rationale: `xG ${m.home}: ${(s.home_xg || 0).toFixed(2)} vs ${(s.away_xg || 0).toFixed(2)}. Превосходство в атаке.`,
      odds: m.odds?.draw,
    });

  if (projGoals > 2.8 && (s.home_xg || 0) + (s.away_xg || 0) > 1.8 && m.odds?.over25 < 2.8)
    signals.push({
      type: 'over', market: 'over_2.5',
      confidence: Math.min(78, 50 + projGoals * 5),
      label: 'Темп матча — Тотал Больше 2.5',
      rationale: `Проекция: ${projGoals.toFixed(1)} гола. Суммарный xG: ${((s.home_xg || 0) + (s.away_xg || 0)).toFixed(2)}.`,
      odds: m.odds?.over25,
    });
  ...
  if (diff >= 2 && pH > pA)
    signals.push({
      ...
      rationale: `Разрыв ${diff} гола + доминирование в атаке. Ставка на победу хозяев.`,
      odds: m.odds?.home,
    });
...
router.get('/matches', async (req, res) => {
  ...
  res.json({ matches: list, liveCount: list.filter(m => m.status === 'live').length,
             total: list.length, lastUpdate: CACHE.ts, source: APIFB_KEY ? 'api-football' : 'demo' });
});
...
router.get('/match/:id', async (req, res) => {
  ...
  res.json({ ...m, oddsHistory: ex.oddsHistory || [], signal: ex.signal });
});
